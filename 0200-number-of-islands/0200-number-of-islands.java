...
class Solution {
    ...
    public int numIslands(char[][] grid) {
        ...
        for(int i=0; i<height; i++) {
            for(int j=0; j<width; j++) {
                if(grid[i][j] == '1' && !isVisited[i][j]) {
                    searchIsland(grid, isVisited, i, j, height, width);
                    islands++;
                }
            }
        }
    ...
    private void searchIsland(char[][] grid, boolean[][] isVisited, int x, int y, int m, int n) {
        if(x>=m || x<0 || y<0 || y>=n || isVisited[x][y] || grid[x][y] == '0') {
            return;
        }

        isVisited[x][y] = true;
        for(int[] direction : DIRECTIONS) {
            searchIsland(grid, isVisited, x+direction[0], y+direction[1], m, n);
        }
    }
}